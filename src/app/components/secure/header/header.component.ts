...
@Component({
    selector: 'app-header',
    standalone: true,
    imports: [IconButtonComponent, CommonModule, NgOptimizedImage],
    template: `
        <div
            id="header"
            class="h-14 px-4 dark:text-zinc-200 my-4 flex flex-row justify-between align-middle"
        >
            <p
                class="inline-flex gap-2 text-2xl my-auto"
                *ngIf="user; else header_skeleton"
            >
                <img
        ...
        <ng-template #header_skeleton>
            <div
                role="status"
                class="max-w-sm rounded shadow animate-pulse md:p-6"
            >
