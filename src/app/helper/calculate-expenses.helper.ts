...
export const calculateExpensesHelper = (
    profile: ProfileDTO
): ExpensesCalculation => {
    if (!profile) return [0, 0, {}, 0];

    const debitAmount = profile.transactions?.reduce(
        (sum, current) => sum + Number(current.amount),
        0
    );
    const recurringExpenseAmount = profile.recurringExpenses?.reduce(
        (sum, current) => sum + Number(current.amount),
        0
    );
    const [creditCardsAmount, creditCardsTotal] = getCreditCardsAmount(
        profile.creditCards
    );
    return [
        debitAmount,
        ...
        creditCardsTotal,
        debitAmount! + creditCardsAmount + recurringExpenseAmount!,
    ];
};

const getCreditCardsAmount = (
    creditCards: CreditCardDTO[]
): [number, CreditCardsTotal] => {
