...
@Component({
    ...
    standalone: true,
    imports: [CurrencyPipe, NgOptimizedImage, DatePipe],
    template: `
        <div
            role="button"
            tabindex="0"
            (keydown)="click()"
            (click)="click()"
            class="w-full flex flex-row justify-between gap-5 items-center align-middle bg-white px-4 py-4 rounded-md border-zinc-200 mb-2"
        >
            <i
                class="{{
...
export default class TransactionCardComponent {
    @Input({ required: true }) transaction!:
        | TransactionDTO
        | RecurringExpenseDTO;
    ...
    getInstallmentLabel(
        transaction: TransactionDTO | RecurringExpenseDTO
    ): string {
