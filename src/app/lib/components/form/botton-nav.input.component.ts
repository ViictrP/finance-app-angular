...
@Component({
  ...
  template: `
    <div class="h-14 mb-10">
      ...
      <app-bottom-sheet title="Escolha uma opção..." #bottomSheet (closed)="showingBottomSheet = false;">
        <div class="flex flex-col gap-2 justify-evenly text-neutral-50">
          @for(option of options; track option.id) {
            <button
              type="button" (click)="optionClicked(option)"
              class="text-zinc-900 w-full p-3 flex flex-row items-center justify-start">
              @if (option.icon) {
                <div class="w-8 h-5 mr-3 rounded-sm">
                  <i class="text-gray-500 text-2xl {{option.icon}}"></i>
                </div>
              } @else {
                <div [ngClass]="option.value" class="w-8 h-5 mr-3 rounded-sm"></div>
              }
              <p>{{ type === 'color' ? colorMap.get(option.id) : option.value }}</p>
            </button>
          }
        </div>
      </app-bottom-sheet>
