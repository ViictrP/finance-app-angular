...
export default class CreditCardService {
    ...
    create(creditCard: CreditCardDTO): Observable<CreditCardDTO> {
        return this.httpClient.post<CreditCardDTO>(
            `${this.apiUrl}/v1/credit-cards`,
            creditCard
        );
    }

    update(creditCard: CreditCardDTO): Observable<CreditCardDTO> {
        return this.httpClient.put<CreditCardDTO>(
            `${this.apiUrl}/v1/credit-cards/${creditCard.id}`,
            creditCard
        );
    }
    ...
    deleteCreditCard(creditCardId: number): Observable<void> {
        return this.httpClient.delete<void>(
            `${this.apiUrl}/v1/credit-cards/${creditCardId}`
        );
    }
}
